...
仅操作主屏任务栏。浏览器和其他应用在前台时默认不透明，不要求最大化；桌面在前台时透明。应用窗口从自身可见区域底部读取属于该窗口的九个像素，按亮度中位数选择浅色或深色。样本不足时尝试窗口声明的深色属性，均不可用时保留上次明暗色但仍变为不透明。开始菜单、任务栏浮层和无效前台暂态保留先前状态。桌面图标与应用位图不重绘，任务栏 XAML 主题用于文字对比度；全屏应用隐藏任务栏时不强制显示。

配置沿用 `FollowMaximizedTheme` / `followMaximizedWindow` 历史字段，以保持旧方案和恢复记录兼容；0.11.9 起其含义为“应用不透明，桌面透明”。旧包仍按其原实现执行，修改源代码不会自动改变正在运行的包。
