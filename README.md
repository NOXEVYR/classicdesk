...
源码与本地 **0.11.5-dev** 已加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)和[登录恢复](docs/LOCAL_RUNTIME.md#登录后继续已有增强)：方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。0.11.3 修复登录时无关注册表更新时间导致的规则误判，并核对完整已应用规则。0.11.5 新增可选的半透明任务栏与紧凑系统托盘，并保留旧运行包的恢复兼容。新增功能尚未发布到下面的 0.10.0-preview 下载包；公开下载继续保留原版本。
