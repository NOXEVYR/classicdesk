...
当前源码及本机设置程序为 **0.11.14-dev**，修正版已暂存到独立服务目录，等待下次重启加载；当前运行的仍为 0.11.13，**本次修复的实机效果尚待验收**；公开下载仍为 **0.10.0-preview**。开发版加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)、透明背景及紧凑系统托盘。方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。
