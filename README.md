...
当前源码及本机设置程序为 **0.11.13-dev**，开机服务已安装并设为下次启动时加载，**冷启动首屏效果尚待实测**；公开下载仍为 **0.10.0-preview**。开发版加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)、透明背景及紧凑系统托盘。方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。
...
0.11.13 使用整机统一布局，服务配置来自已应用方案，设置窗口不参与开机流程。新前端阻止便携引擎重复启动；服务模式的方案更新界面尚待补齐，保存草稿不代表系统规则已更新。本轮 **323 项自动检查通过**，详细状态见 [验证记录](VALIDATION.md)。
