...
仅操作主屏任务栏。0.11.14 起会继续查找普通小窗口后方的最大化或全屏应用，以本屏最上方仍显示的最大化背景决定不透明和颜色；小窗口获得焦点不会清除该背景。最后一个最大化窗口还原、最小化或隐藏后恢复透明。需要不透明时，从窗口自身可见区域底部读取属于该窗口的九个像素，按亮度中位数选择浅色或深色。样本不足时尝试窗口声明的深色属性，均不可用时保留上次明暗色。焦点落在任务栏或系统浮层时检查本屏最上层可见应用，无法判断则保留状态。桌面图标与应用位图不重绘，任务栏 XAML 主题用于文字对比度；全屏应用隐藏任务栏时不强制显示。
