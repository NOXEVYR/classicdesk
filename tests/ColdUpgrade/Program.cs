...
Test("exact applied profile is carried over including all-left menu and explorer settings", h => { h.Source = h.Source with { AppliedProfile = new ShellProfile(StartOnLeft: false, IconSize: 28, ClassicRibbon: false, ClassicContextMenu: false, TranslucentTaskbar: true, FollowMaximizedTheme: true, LeftAlignedApps: true), EngineExited = true }; var t = h.Ticket(); h.Core.Apply(t); Check(h.StartedProfile == t.Source.AppliedProfile && h.StartedProfile.LeftAlignedApps); });
