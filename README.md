...
当前源码为 **0.11.10-dev**，本机已启用版本为 **0.11.9-dev**，公开下载仍为 **0.10.0-preview**。开发版加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)、透明背景及紧凑系统托盘。方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。

0.11.9 的新规则是：**浏览器或其他应用在前台时任务栏不透明，切回桌面时透明**；普通窗口也适用，保留浅色/深色配色。重启后已在干净 Explorer 中启用，用户确认点击桌面与切回浏览器可切换，但最小化后仍需再点桌面。0.11.10 补充最小化/隐藏事件及提前加载时的初始化，独立构建尚未替换实机。48 DIP 栏高、24 DIP 应用图标、开始靠左、应用居中及右侧紧凑结构保留。详见[任务栏行为与验收记录](docs/TASKBAR_BEHAVIOR.md)。
