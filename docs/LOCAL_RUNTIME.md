...
开发中的 0.11 阶段只启用任务栏分开布局和尺寸参数。资源管理器与右键菜单设置仍可编辑、保存、预览，但本次启用会将这两个模块保持停用。不会改写用户保存的完整方案。当前尚未完成真实桌面效果与性能验收。
...
准备后四个模块均为 `Disabled=1`，主程序和引擎均为 `SafeMode=1`；更新检查、托盘和工具窗口保持停用。要实际试用，打开此构建的设置窗口，进入“系统应用检查”，核对提示，再点击“试用任务栏”。没有无人值守启用命令，也不会添加开机启动。
...
首次现场尝试中，StartAllBack 3.9.25 在 Windows 11 25H2 上勾选“为当前用户禁用该程序”后，单独重启 Explorer 仍有完整组件加载；其设置页要求注销。遇到这一情况应保持冲突阻断，由用户保存工作并注销重新登录，不能仅凭注册表停用值就忽略实际加载模块。此次尝试已撤销临时停用，保持原桌面配置。
