...
当前源码为 **0.11.9-dev**，公开下载仍为 **0.10.0-preview**。开发版加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)、[登录恢复实现](docs/LOCAL_RUNTIME.md#登录后继续已有增强)、透明背景及紧凑系统托盘。方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。

0.11.9 的新规则是：**浏览器或其他应用在前台时任务栏不透明，切回桌面时透明**；普通窗口也适用，保留浅色/深色配色。源码和独立测试包已完成，尚未应用到实机。48 DIP 栏高、24 DIP 应用图标、开始靠左、应用居中及右侧紧凑结构的目标保留。详见[任务栏行为与验收记录](docs/TASKBAR_BEHAVIOR.md)。

**开发版仍有已知故障，尚不能完整替代 StartAllBack。** 0.11.8 实机重启后背景组件未成功加载，组件重载期间出现 Explorer 和开始菜单进程崩溃；0.11.9 本轮修改规则，未修复这些加载/重载问题。登录自动接续在测试机器上仍关闭，不能承诺每次开机自动生效。测试通过与引擎正在运行均不代表 Windows 外观已验证。公开下载不包含这些新增功能或第三方增强运行组件。
