...
当前源码及本机设置程序为 **0.11.14-dev**，重启后已确认新版服务自动运行，六项增强模块加载且动态背景初始化正常；**用户已确认后台最大化窗口与前台小窗口叠放、最小化后的透明切换正常**；公开下载仍为 **0.10.0-preview**。开发版加入[本地方案库](docs/PROFILE_LIBRARY.md)、[按需启用功能](docs/FEATURE_SELECTION.md)、透明背景及紧凑系统托盘。方案可命名保存、更新、复制、重命名和归档；任务栏布局、尺寸、资源管理器和右键菜单可分别选择应用。
