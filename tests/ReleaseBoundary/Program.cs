...
var earlyAdaptiveBytes = File.ReadAllBytes(Path.Combine(sourceRoot.FullName, "runtime", "windows-x64-assets-adaptive-v5.json"));
allChecks.Add(new { name = "Early-init adaptive package keeps its recovery pin", passed = Convert.ToHexString(System.Security.Cryptography.SHA256.HashData(earlyAdaptiveBytes)) == ShellNativeController.FifthAdaptiveRuntimeManifest });
